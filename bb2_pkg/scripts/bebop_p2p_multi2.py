...
class MoveByGPS:
	...
	def save_starting_point(self):
		...
		if self.isflagsavegps == True:
			...
			print(self.saveGPS[0], self.saveGPS[1], self.saveGPS[2], self.saveGPS[3])
	...
	def move_to_target(self, g_lati, g_long, g_alti, speed):
		...
		while not rospy.is_shutdown():
			...
			if dist < LIMITE_DIST or dist_s1 < LIMITE_DIST:
				break
	...
	def takeoff(self):
		...
		self.pub4.publish(self.empty_msg);  print "takeoff slave";    rospy.sleep(0.5);
		self.pub4.publish(self.empty_msg);  rospy.sleep(3.0)

   
	def landing(self):
		self.pub2.publish(self.empty_msg);  print "landing master"
		self.pub5.publish(self.empty_msg);  print "landing slave"
...
if __name__ == '__main__':
	try:
		...
		print "the GPS coordination slave (%s, %s)" %(mbg.saveGPS[2], mbg.saveGPS[3])
